...
function DeployEigen3( [string] $downloadDirectory, [string] $targetDir )
{
    $eigenSourceDir = Join-Path $downloadDirectory "\eigen-unzip"
    $eigenDestinationDir = Join-Path $targetDir "\eigen3"
    Request-DirectoryExists $eigenDestinationDir
...
$BuildRootDirectory = Join-Path $PSScriptRoot "\temp-build"
Request-DirectoryExists $BuildRootDirectory

$DownloadRootDirectory = Join-Path $BuildRootDirectory "\downloads"
...
Push-Location $BuildRootDirectory
    ...
    DownloadAndExtractEigen3 $BuildRootDirectory $DownloadRootDirectory $7ZipDirectory
	
	DeployEigen3 $BuildRootDirectory $PSScriptRoot	
